...
export async function initUpdate() {
    ...
    // Get version from sysinfo instead of version.txt
    let filesystemversion = "unknown";
    try {
        const versionResponse = await fetch("/sysinfo");
        const sysinfo = await versionResponse.json();
        filesystemversion = sysinfo.filesystemversion || sysinfo.buildversion || "unknown";
    } catch (error) {
        console.warn("Could not get version from sysinfo:", error);
