...
export function persistSavedDevices(store: SavedDevicesStore) {
  writeJSON(STORE_KEY, store.list.map(d => ({ id: d.id, name: d.name, host: d.host, port: d.port, com: d.com, method: d.method })));
...
export function addSavedDevice(store: SavedDevicesStore, dev: Omit<SavedDevice, 'id'> & Partial<Pick<SavedDevice, 'id'>>): SavedDevicesStore {
  ...
  const rec: SavedDevice = { id, name: dev.name || id, host: dev.host || '', port: dev.port || '', com: dev.com || '', method: dev.method || 'http' };
