...
class SerialManager {
    ...
    return new Promise<SerialState>((resolve, reject) => {
      try {
        this.port = new SerialPort({ path, baudRate }, (err) => {
          if (err) {
            try { this.port?.removeAllListeners(); this.port = null; } catch { /* ignore */ }
            return reject(err);
          }
          // Only mark open after success
          ...
          this.port!.on('error', err2 => {
            const ts = new Date().toISOString();
            const classified: ClassifiedLine = { idx: this.state.nextIndex++, ts, raw: `<error> ${err2.message}`, type: 'ERROR' };
            this.logBuffer.push(classified);
          });
          this.port!.on('close', () => {
            this.state.isOpen = false;
          });
          // slight delay to allow device to finish any banner output before considered ready
          setTimeout(() => resolve(this.getState()), 80);
        });
      } catch (e) { reject(e); }
    });
  }
  ...
  async execCli(command: string, timeoutMs = 1500) {
    // Defensive: sometimes underlying SerialPort may report closed while our state still shows open
    if (!this.port || !this.port.isOpen) {
      if (this.state.portPath) {
        try {
          // Attempt a fast transparent reopen using last known settings
          await this.open(this.state.portPath, this.state.baudRate);
        } catch (re) {
