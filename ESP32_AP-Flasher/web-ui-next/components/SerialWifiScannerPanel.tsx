...
export const SerialWifiScannerPanel: React.FC<SerialWifiScannerPanelProps> = ({ onSelectSSID }) => {
  const { networks, scanning, error, scan, lastCount, timedOut } = useSerialWifiScan();
  return (
    <div className="border rounded p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold">Serial WiFi Scan</h3>
        <button disabled={scanning} onClick={()=>scan()} className="px-3 py-1 rounded bg-blue-600 text-white disabled:opacity-50">
          {scanning? 'Scanning...':'Scan'}
        </button>
      </div>
      {error && <div className="text-red-600 text-sm">Error: {error}</div>}
      {lastCount !== undefined && <div className="text-xs text-gray-500">Found {networks.length} network(s){timedOut && ' (timeout)'}.</div>}
