...
export function useSavedDevices(){
  ...
  const add = useCallback((input: { name: string; host?: string; com?: string; method?: 'http'|'ws'|'serial' })=>{
    setStore(s=> addSavedDevice(s, input));
  },[]);
  const update = useCallback((id: string, patch: Partial<{ name: string; host: string; com: string; method: 'http'|'ws'|'serial' }>)=>{
