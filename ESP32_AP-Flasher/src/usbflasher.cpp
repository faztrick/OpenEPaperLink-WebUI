    ...
    while (len--)
    {
        ...
        case FLASHER_WAIT_CRCL:
            ...
            else
            {
                if (transportType == TRANSPORT_USB)
                {
                    // Guard: if queue not yet created, drop to avoid early-boot pxQueue assert
                    if (flasherCmdQueue)
                    {
                        BaseType_t queuestatus = xQueueSend(flasherCmdQueue, &cmd, 0);
                        if (queuestatus == pdFALSE)
                        {
                            // queue full; drop command to avoid blocking/assert
                            if (cmd != nullptr)
                            {
                                if (cmd->data)
                                {
                    ...
                    {
                        // Queue not ready yet: drop safely
                        if (cmd != nullptr)
                        {
                            if (cmd->data)
                            {
