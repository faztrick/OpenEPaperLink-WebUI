...
#define LOG(format, ...)                                         \
    do {                                                         \
        ...
        LogUtils::logInfo(String(buffer));                       \
    } while (0)
...
bool downloadAndWriteBinary(String &filename, const char *url) {
    ...
    if (!buffer) {
        wsSerial("Failed to allocate download buffer");
        return false;
    }
    ...
    do {
        ...
        while (stream->connected() && totalBytesRead < contentLength) {
            ...
            size_t bytesToRead;
            if (stream->available()) {
                bytesToRead = std::min((size_t)1024, (size_t)stream->available());
    ...
    if (Ret == true) do {
            ...
            for (JsonObject obj : jsonArray) {
                ...
                for (int retry = 0; retry < maxRetries; retry++) {
                    ...
                    LogUtils::logWarning("Flash failed with error " + String(err) + ". Retrying...");
                    delay(1000);
                }
                if (err != ESP_LOADER_SUCCESS) {
                    break;
