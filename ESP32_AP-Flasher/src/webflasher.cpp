...
{
    // Defer starting the TCP server until the network stack is ready.
    // Starting AsyncServer too early can cause asserts inside lwIP/FreeRTOS.
    ...
    while (1)
    {
        if (!tcpStarted)
        {
            ...
            if (net_ready)
            {
                ...
                tcpStarted = true;
                Serial.println("[WEBFLASHER] TCP AsyncServer started on port 243");
            }
            else
            {
        ...
        case AUTOFLASH_STEP_IDLE:
        {
            if (webFlashMode != FLASHMODE_OFF && (tagConnectTimer.doRun() || webFlashMode == FLASHMODE_AUTO_FOCUS))
            {
                ...
                if (extTagConnected())
                    autoFlashStep = AUTOFLASH_STEP_CONNECT;
            }
            break;
        }
        ...
        case AUTOFLASH_STEP_STARTUP:
        {
            ...
            if (webFlashMode == FLASHMODE_AUTO_BACKGROUND)
                tftOverride = false;
#endif
...
void handleWSdata(uint8_t *data, size_t len, AsyncWebSocketClient *client)
{
    ...
    if (doc["flashcmd"].is<int>())
    {
        ...
        case WEBFLASH_FOCUS:
            ...
            break;
        case WEBFLASH_BLUR:
            if (webFlashMode == FLASHMODE_AUTO_FOCUS)
                webFlashMode = FLASHMODE_AUTO_BACKGROUND;
#ifdef HAS_TFT
            tftOverride = false;
            sendAvail(0xFC);
#endif
