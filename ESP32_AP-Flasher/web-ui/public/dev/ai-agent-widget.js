...
(function(){
    ...
    async function sendMessage() {
        ...
        // If the dev server exposes a /wwwroot-list or /sysinfo endpoint, we can call it
        try {
            if (text.toLowerCase().includes('list files')) {
                const r = await fetch('/wwwroot-list');
                if (r.ok) { const json = await r.json(); const bot = document.createElement('div'); bot.className='ai-msg bot'; bot.textContent = 'Files: ' + (Array.isArray(json.files)? json.files.slice(0,20).join(', '): JSON.stringify(json)); body.appendChild(bot); body.scrollTop = body.scrollHeight; return; }
            }
        } catch(e) {
            // ignore
        }
        ...
        // Fallback responses
        let reply = 'I am a local assistant widget. For full AI features open /ai-agent.html.';
        if (text.toLowerCase().includes('status')) reply = 'Check the dashboard or the /sysinfo endpoint for system status.';
        const bot = document.createElement('div'); bot.className='ai-msg bot'; bot.textContent = reply; body.appendChild(bot); body.scrollTop = body.scrollHeight;
    }
