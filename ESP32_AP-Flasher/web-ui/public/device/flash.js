...
export function wsCmd(command) {
    ...
    // Prefer apiManager if present (it provides queueing)
    if (window.apiManager && typeof window.apiManager.sendWS === 'function') {
        window.apiManager.sendWS(jsonData).catch(err => {
            print('Failed to send websocket message: ' + err, 'red');
        });
        return;
    }
    ...
    if (window.socket && window.socket.readyState === WebSocket.OPEN) {
        ...
        // Queue using the global socket check: attempt resend when socket opens
        print("WebSocket not connected, message queued", "yellow");
        ...
        setTimeout(() => clearInterval(trySend), 30000);
    }
}
