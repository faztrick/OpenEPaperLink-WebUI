...
// Use OptimizedApp from app-core.js instead of duplicating
...
function connect() {
	...
	socket.addEventListener("message", (event) => {
		...
		try {
			...
			if (msg.logMsg) showMessage(msg.logMsg, false);
			if (msg.errMsg) showMessage(msg.errMsg, true);
			if (msg.tags) processTags(msg.tags);

			// Some messages may carry wifi/status info either at top-level or under a 'data' property.
			const data = msg.data || msg;

			if (data && data.rssi !== undefined && data.rssi !== null) {
				const rssi = Number(data.rssi);
				const signalPercent = Math.max(0, Math.min(100, (rssi + 100) * 2));
				updateMetric('wifi-signal', signalPercent, 100, '%');

				const wifiTextEl = document.getElementById('wifi-text');
				if (wifiTextEl) wifiTextEl.textContent = rssi + ' dBm';

				const signalFill = document.getElementById('wifi-signal');
				if (signalFill) {
					signalFill.style.width = signalPercent + '%';
					let level = 'critical';
					if (rssi > -50) level = 'low';
					else if (rssi > -65) level = 'medium';
					else if (rssi > -80) level = 'high';
			...
			// Update AP / runstate display if present
			if (msg.sys) {
...
document.querySelectorAll('.closebtn').forEach(button => {
	button.addEventListener('click', (event) => {
		...
		{ const adv = $('#advancedoptions'); if (adv) adv.classList.add('collapsed'); }
	});
});
...
if (cfgMoreElement) {
	...
	if (!adv) return;
	const isCollapsed = adv.classList.toggle('collapsed');
	$('#cfgmore').innerHTML = isCollapsed ? '&#x25B2;' : '&#x25BC;';
	// Use CSS transition (max-height) instead of direct height manipulation to avoid reflow loops
	};
}
...
// Content generation functionality removed from UI; initialization continues below
// Initialize simple file flash functionality when DOM is ready
document.addEventListener('DOMContentLoaded', function() {
        ...
        flashFileInput.onchange = function (event) {
            ...
            uploadFlashFile(file)
                .then(() => {
					...
					// Use apiManager.sendWS if available to queue messages while reconnecting
					if (window.apiManager && typeof window.apiManager.sendWS === 'function') {
						window.apiManager.sendWS(JSON.stringify(flashCmd)).catch(err => { throw err; });
					} else if (socket && socket.readyState === WebSocket.OPEN) {
						socket.send(JSON.stringify(flashCmd));
					} else {
						// Fallback: show user-friendly error instead of throwing
						showMessage('WebSocket not connected, please wait for connection', true);
...
function togglePerformanceDetails() {
	...
	if (!details) return;
	// Use requestAnimationFrame to batch DOM updates and avoid layout-measure loops
...
function updatePerformanceMetrics() {
	...
	fetch('/get_wifi_config')
		.then(response => response.json())
		.then(data => {
			// If RSSI is present, compute a 0-100 percent for the bar and show dBm text
			if (data && data.rssi !== undefined && data.rssi !== null) {
				const rssi = Number(data.rssi);
				const signalPercent = Math.max(0, Math.min(100, (rssi + 100) * 2));
				// Update the visual bar using percent
				updateMetric('wifi-signal', signalPercent, 100, '%');

				// Textual RSSI display
				const wifiTextEl = document.getElementById('wifi-text');
				if (wifiTextEl) wifiTextEl.textContent = rssi + ' dBm';
...
const WiFiSetup = {
	...
	getSsidList() {
		...
		window.apiManager.unifiedScan({ timeoutMs: 15000, pollInterval: 800 })
			.then(res => {
				...
				networks.forEach((n, idx) => {
					if (!n.ssid || !n.ssid.trim()) return;
					const option = document.createElement('option');
					option.value = n.ssid;
					const rssiText = this.formatSignalStrength(n.rssi);
					const ssidText = this.pad(n.ssid, 28);
					const securityText = this.formatSecurity(n.enc || n.encryption);
					const channelText = `Ch${n.channel}`;
					option.text = `${rssiText} ${ssidText} ${securityText} ${channelText}`;
					option.title = `SSID: ${n.ssid}, Signal: ${n.rssi}dBm, Channel: ${n.channel}, Security: ${this.getSecurityType(n.enc || n.encryption)}`;
					select.appendChild(option);
					if (idx < 5) console.log('Added network:', n);
				});
				...
				let previous = $('#ssid') ? $('#ssid').value : '';
				const currentSSIDInput = $('#ssid');
				if (currentSSIDInput) {
					currentSSIDInput.replaceWith(select);
					setTimeout(() => {
						const newSelect = $('#ssid');
						if (newSelect && previous) {
							for (let i = 0; i < newSelect.options.length; i++) {
								if (newSelect.options[i].value === previous) {
									newSelect.selectedIndex = i; break;
								}
							}
						}
					}, 10);
				}
				this.showStatus('scan_status', `Found ${networks.length} WiFi networks. Select one from the dropdown.`, 'success');
    ...
    tryFallbackScan() {
        console.log('Trying fallback (cached unified results)...');
        window.apiManager.getWifiScanResults()
            .then(res => {
                const networks = res.networks || [];
                if (networks.length === 0) {
                ...
                const current = $('#ssid');
                let prev = current ? current.value : '';
                if (current) current.replaceWith(select);
                if (prev) {
                    setTimeout(() => {
                        const s = $('#ssid');
                        if (s) {
                            for (let i = 0; i < s.options.length; i++) if (s.options[i].value === prev) { s.selectedIndex = i; break; }
                        }
                    }, 10);
                }
                this.showStatus('scan_status', `Found ${networks.length} WiFi networks (cached path).`, 'success');
                this.resetScanButton();
            })
            .catch(err => {
                console.error('Fallback scan failed:', err);
                this.showStatus('scan_status', 'All scan methods failed. Enter SSID manually.', 'error');
                this.resetScanButton();
            });
    },
};
