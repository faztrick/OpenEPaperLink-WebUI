...
extern void processBlockRequest(espBlockRequest* br);
...
extern void prepareExternalDataAvail(pendingData* pending, IPAddress remoteIP);
extern void processXferComplete(espXferComplete* xfc, bool local);
extern void processXferTimeout(espXferComplete* xfc, bool local);
extern void processDataReq(espAvailDataReq* adr, bool local, IPAddress remoteIP = IPAddress(0, 0, 0, 0));
extern void processTagReturnData(espTagReturnData* trd, uint8_t len, bool local);
...
bool checkMirror(tagRecord* taginfo, pendingData* pending);
...
bool queueDataAvail(pendingData* pending, bool local);
